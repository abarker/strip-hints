...
def check_whited_out_line_breaks(token_list, rpar_and_colon=None):
    ...
    for t in token_list:
        if t.type_name == "NL":
            if fix_linebreaks_in_return_by_moving_colon and rpar_and_colon:
                if moved_colon:
                    continue
...
def process_return_part(return_part, rpar_token):
...
def process_funcdef_without_suite(funcdef_logical_line):
    ...
    process_return_part(split_on_parens[2], rpar_token=rpar) # The return part.
...
def process_command_line():
    ...
    # Really needs argparse, maybe options dict.
    # Globals below so entry-point fun can take no args but change options.
    global map_hints_to_empty_strings
    ...
    if "--no-colon-move" in sys.argv:
        fix_linebreaks_in_return_by_moving_colon = False
